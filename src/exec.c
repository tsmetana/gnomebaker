...
/*	
	...
	if(fpin == NULL)
	{
		g_critical("exec_run_cmd - popen error");
	}
	else
	{
		ret = g_string_new("");
		static const gint MAXLINE = 1024;
		gchar tmp[MAXLINE];
		memset(tmp, 0, MAXLINE);

		while((fgets(tmp, MAXLINE, fpin) != NULL))
		{
		...
		if(pclose(fpin) == -1)
		{
			g_critical("exec_run_cmd - pclose error");
		}
	}
*/	

	/*
	gint stdin = 0, stdout = 0, stderr = 0;

	
	gboolean result = g_spawn_async_with_pipes(
		NULL, e->argv, NULL, G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD 
		| G_SPAWN_FILE_AND_ARGV_ZERO,
		NULL, NULL, &e->pid, &stdin, &stdout, &stderr, &ex->err);

	g_message("spawed process with pid [%d]", e->pid);

	if(result)
	{
		static const gint BUFF_SIZE = 256;
		gchar buffer[BUFF_SIZE];
		memset(buffer, 0, BUFF_SIZE);				
										
		while(read(stdout, buffer, BUFF_SIZE) > 0);
		{
			g_message( "Exec stdout - %s", buffer);
			if(e->readProc)
				e->readProc(ex, buffer);
			memset(buffer, 0, BUFF_SIZE);
		}
		
		while(read(stderr, buffer, BUFF_SIZE) > 0)
		{
			g_message( "Exec stderr - %s", buffer);
			if(e->readProc)
				e->readProc(ex, buffer);
			memset(buffer, 0, BUFF_SIZE);
		}

		g_message( "exec_thread - closing descriptors");
		close(stdin);
		close(stdout);
		close(stderr);

		g_message( "exec_thread - waitpid");
		waitpid(e->pid, &e->exitCode, 0);
		g_message("exec_thread - child exitcode [%d]", e->exitCode);
	}
	else
	{
		g_critical("exec_thread - failed to spawn process [%d] [%s]",
