...
datacd_compilation_root_add()
{
    ...
    gtk_tree_store_append(datacd_compilation_store, &root_iter, NULL);           
	gtk_tree_store_set(datacd_compilation_store, &root_iter, DATACD_COL_ICON, icon, 
...
static void
datacd_list_view_clear(GtkTreeView *file_list)
{
	GB_LOG_FUNC
...
datacd_list_view_update(GtkTreeIter *parent_iter)
{
	...
	GtkTreeView *file_list = GTK_TREE_VIEW(glade_xml_get_widget(gnomebaker_getxml(), widget_datacd_list));
	GtkListStore *store = GTK_LIST_STORE( gtk_tree_view_get_model(file_list) ); 	
	datacd_list_view_clear(file_list);
 	                                        ...
 	                                        child_number))
    {
		...
		GtkTreeRowReference *row_reference = gtk_tree_row_reference_new(GTK_TREE_MODEL(datacd_compilation_store),path);		
		gtk_tree_path_free(path);					
...
datacd_get_datadisk_size()
{
	...
	data_disk_size = data_disk_sizes[gtk_option_menu_get_history(GTK_OPTION_MENU(option_menu))].size;
	return data_disk_size;
}


static gchar*
datacd_format_progress_text(gdouble current_size)
{
    GB_LOG_FUNC
...
datacd_update_progress_bar()
{
	...
	GtkWidget *progress_bar = glade_xml_get_widget(xml, widget_datacd_progressbar);
	g_return_if_fail(progress_bar != NULL);
	
	const gdouble disk_size = datacd_get_datadisk_size();	
...
static gboolean  
datacd_add_to_compilation(const gchar *file, GtkTreeIter *parent_node, gboolean existing_session)
{
	GB_LOG_FUNC
	...
	if(statret == 0)
	{
		...
		GB_DECLARE_STRUCT(GtkTreeIter, iter);
		gtk_tree_store_append(datacd_compilation_store, &iter, parent_node);	
		...
		{ 
			...
			if(dir != NULL)
			{
				...
				while(name != NULL)
				{
					...
					if(!datacd_add_to_compilation(full_name, &iter, existing_session))
					{
						g_free(full_name);
						break;							
					}
								  ...
								  GList **rref_list)
{
	...
	int count=0;
	GB_DECLARE_STRUCT(GtkTreeIter, child_iter)
	while(gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(datacd_compilation_store),&child_iter,parent_iter,count))
	{
...
static void
datacd_foreach_fileselection(GtkTreeModel *file_model,
								  GtkTreePath *path,
								  GtkTreeIter *iter,
								  gpointer user_data)
{
	...
	if(GTK_IS_LIST_STORE(file_model))
	{
		...
		if(global_path!=NULL)
		{
			*rowref_list = g_list_prepend(*rowref_list, row_reference);			
...
datacd_remove(GtkTreeView *widget)
{
	...
	else
	{
		...
		if(!datacd_compilation_is_root(&global_iter))
		{
			...
			rr_list = g_list_prepend(rr_list, row_reference);	
		...
		if(node->data!=NULL)
		{
			...
			if (path!=NULL)
			{
				...
				if (gtk_tree_model_get_iter(GTK_TREE_MODEL(datacd_compilation_store), &iter, path))
				{
					...
					gtk_tree_model_get_value(GTK_TREE_MODEL(datacd_compilation_store), &iter, DATACD_COL_SESSION, &session_value);
...
static void
datacd_on_remove_clicked(GtkWidget *menuitem, gpointer user_data)
{
	GB_LOG_FUNC
	
	g_return_if_fail(user_data != NULL);
	datacd_remove(GTK_TREE_VIEW(user_data));		
}
...
static void
datacd_on_clear_clicked(GtkWidget *menuitem, gpointer user_data)
{	
	GB_LOG_FUNC
	datacd_clear();
}
...
static void
datacd_tree_sel_changed(GtkTreeSelection *selection, gpointer user_data)
{
	GB_LOG_FUNC
	g_return_if_fail(selection != NULL);
...
static void
datacd_setup_tree( GtkTreeView *dir_tree)
{
	GB_LOG_FUNC
	...
	/*g_signal_connect(G_OBJECT(dir_tree), "row-activated", G_CALLBACK(datacd_on_tree_dbl_click), NULL);*/
}
...
static void
datacd_setup_list( GtkTreeView *file_list)
{
	GB_LOG_FUNC
	g_return_if_fail(datacd_compilation_store != NULL);
	...
	g_signal_connect(content_renderer, "edited", (GCallback)datacd_contents_cell_edited, 
		(gpointer)store);
	...
	g_signal_connect(G_OBJECT(file_list), "row-activated", G_CALLBACK(datacd_on_tree_dbl_click), NULL);
}
...
datacd_new()
{
	...
	GtkTreeView *dir_tree = GTK_TREE_VIEW(glade_xml_get_widget(
		gnomebaker_getxml(), widget_datacd_tree));
	GtkTreeView *file_list = GTK_TREE_VIEW(glade_xml_get_widget(
		gnomebaker_getxml(), widget_datacd_list));
	
	g_return_if_fail(dir_tree != NULL);	
	g_return_if_fail(file_list != NULL);	
	
	datacd_setup_list(file_list);
	datacd_setup_tree(dir_tree);
	datacd_compilation_root_add();
...
datacd_on_datadisk_size_changed(GtkOptionMenu *option_menu, gpointer user_data)
{
	GB_LOG_FUNC
...
static void
datacd_build_filepaths_recursive(GtkTreeModel *model, GtkTreeIter *parent_iter, const char  *file_path ,GList **compilation_list)
{
    GB_LOG_FUNC
     ...
     */
     ...
     if(gtk_tree_model_iter_has_child(model, &root_iter))
     {
        ...
        while(gtk_tree_model_iter_nth_child(model,&iter,&root_iter,count))
        {
            ...
            {
                datacd_build_filepaths_recursive(model, &iter, file_name ,&compilation_list);
...
datacd_on_create_datadisk(gpointer widget, gpointer user_data)
{
	...
	if(data_disk_size >= data_disk_sizes[DVD_4GB].size)
    {
        if(msinfo != NULL)
...
datacd_save_project()
{
    ...
    GtkWidget *data_tree = glade_xml_get_widget(gnomebaker_getxml(), widget_datacd_tree);
    ...
    gtk_tree_model_foreach(data_model, (GtkTreeModelForeachFunc) datacd_foreach_save_project, project);
