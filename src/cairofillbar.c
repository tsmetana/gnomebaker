...
static gboolean gb_cairo_fillbar_expose(GtkWidget *widget, GdkEventExpose *event);	
...
gb_cairo_fillbar_class_init (GBCairoFillBarClass *klass)
{
	...
	widget_class->expose_event = gb_cairo_fillbar_expose;
}
...
static void
shade (const CairoColor *in,
	CairoColor *out, float k)
{
	double red;
	double green;
	double blue;
						...
						gdouble tolerance)	   /*the size to fill in yellow*/
{
	...
	total_rect.width  = (gint)(current_size*one);
	total_rect.x = real_x;
	total_rect.y = real_y;
...
gb_cairo_fillbar_get_current_text (GBCairoFillBar *bar)
{
	...
	if(bar->priv->is_time)
	{
		  ...
		  return g_strdup_printf("%d mins %d secs used - %d mins %d secs remaining",
									mins, 
									secs,
									mins_remaining,
									secs_remaining);
	...
	{		        
			...
			(bar->priv->project_total_size < ( bar->priv->disk_size *( 1 + bar->priv->tolerance_percent))))
    	{
        	...
        	buf = g_strdup_printf( "%s of %s used. <b>Overburning</b>.", current, remaining);
    	}
    	else if(bar->priv->project_total_size < bar->priv->disk_size)
    	{
        	...
        	remaining = gbcommon_humanreadable_filesize( bar->priv->disk_size - bar->priv->project_total_size );
       		buf = g_strdup_printf( "%s used - %s remaining.", current, remaining);
   		}
   		else
   	 	{
        	current = gbcommon_humanreadable_filesize( bar->priv->project_total_size );
        	remaining = gbcommon_humanreadable_filesize((bar->priv->disk_size));
        	buf = g_strdup_printf( "%s of %s used. <b>Disk full</b>.", current, remaining);
    	}
    	g_free(current);
    	g_free(remaining);
    	return buf;
							...
							gint amount)
{
	...
	GdkRectangle prelight_clip, normal_clip;
  	...
  	text_buf = gb_cairo_fillbar_get_current_text (bar);
  
  	layout = gtk_widget_create_pango_layout (widget, text_buf);
...
gb_cairo_fillbar_expose(GtkWidget *widget, GdkEventExpose *event)
{
	GB_LOG_FUNC
	cairo_t *cr;	 
	cr = gdk_cairo_create(widget->window);  /*Get a cairo context*/
	...
	gdouble max_value = 0;
	gdouble max_disk_size = 0;
	gdouble current_size = 0;
	gdouble tolerance = 0;	
	...
	if(bar->priv->is_time)
	{
		...
		max_value = (max_disk_size > current_size ? max_disk_size : current_size) + 10;
	...
	else
	{
		...
		max_value = (max_disk_size > current_size ? max_disk_size : current_size) + 100;
		tolerance = bar->priv->tolerance_percent * max_disk_size;
	}
	...
	/*paint text*/
		int amount = (widget->allocation.width - 2 * widget->style->xthickness) *
